...
export default function NotebookEditor() {
  ...
  const [selectedTheme, setSelectedTheme] = useState(notebookThemes[0]);
  ...
  const { data: project, isLoading } = useQuery<WritingProject>({
    queryKey: ["writing-project", projectId],
    queryFn: () => apiClient.getWritingProject(projectId!),
    enabled: !!projectId,
  });
  ...
  useQuery({
    ...
    onSuccess: (data) => {
      ...
      const theme =
        notebookThemes.find((t) => t.id === data.theme) || notebookThemes[0];
  ...
  const saveContentMutation = useMutation({
    mutationFn: (data: { content: string; wordCount: number; theme: string }) =>
      apiClient.saveNotebookContent(projectId!, data),
    onSuccess: () => {
        ...
        >
          <div className="flex items-center gap-6">
            ...
            {project.targetWordCount && (
              <span>
                {Math.round((wordCount / project.targetWordCount) * 100)}% of
                target
              </span>
            )}
          </div>
